...
export const getComponentTypes = async () => {
  const rows = await db.select({ componentType: componentTypes, component: components }).from(componentTypes).leftJoin(components, eq(componentTypes.id, components.type));
...
export const getComponentTypeById = async (id: ComponentTypeId) => {
  ...
  const [row] = await db.select({ componentType: componentTypes, component: components }).from(componentTypes).where(eq(componentTypes.id, componentTypeId)).leftJoin(components, eq(componentTypes.id, components.type));
